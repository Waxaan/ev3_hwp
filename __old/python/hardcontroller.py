...
class HardController:
    ...
    def forward(self, distance):
        try:
            self._tank.on_for_rotations(SpeedPercent(
                DRIVE_SPEED), SpeedPercent(DRIVE_SPEED), distance)
        except KeyboardInterrupt:
            self.stop()
            raise

    def forwardUS(self, distance):
        speed = DRIVE_SPEED
        if distance < 0:
            distance *= -1
            speed -= 1
        try:
            start = self.getDistance()
            self._tank.on(SpeedPercent(
                speed), SpeedPercent(speed))
            while self.getDistance() - start > distance:
                time.sleep(0.01)
            self.stop()
        except KeyboardInterrupt:
            self.stop()
    ...
    def getScan(self):
        result = []
        self.reset()
        self._tank.on(-SCAN_SPEED, SCAN_SPEED)
        last = None
        try:
            while self.getAngle() < 360:
                time.sleep(0.01)
                # pause > 250ms between us usage
                if last == None or time.time() - last > 250:
                    last = time.time()
                    result.append((self.getAngle(), self.getDistance()))
        except KeyboardInterrupt:
            self.stop()
            raise
        self.stop()
